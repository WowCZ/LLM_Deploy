...
class ArenaPlan():
    def __init__(self, 
        ...
        self.show_head = 0
        self.annotated_matches = dict()
        self.annotated_names = list()
    ...
    def _show_annotated_match(self, match_id: int):
        game_name = self.annotated_names[match_id]
        match = self.annotated_matches[game_name]

        instruct = match['instruction']
        model_a = match['model-a']
        model_b = match['model-b']
        winner = match['winner']
    ...
    def progress(self):
        return f'{self.show_head+1}/{self.match_cnt}'
    
    def submit_match(self, match_result: dict):
        game_name = list(self.matches[self.show_head].keys())[0]
        if game_name not in self.annotated_matches:
            self.annotated_names.append(game_name)
        self.annotated_matches[game_name] = match_result
...
def next_match():
    global arena_plan
    _, instruct, model_a, model_b, winner = arena_plan.next_match()
    return instruct, model_a, model_b, winner, _annotation_result(winner)

def last_match():
    global arena_plan
    _, instruct, model_a, model_b, winner = arena_plan.last_match()
    return instruct, model_a, model_b, winner, _annotation_result(winner)
...
def load_match(ability, llms):
    ...
    _, instruct, model_a, model_b, winner = arena_plan.show_match()

    return f'{len(match_plan)} battles have been loaded!', instruct, model_a, model_b, winner, _annotation_result(winner)

def save_result():
    global arena_plan
    arena_plan.save_matches()

def _annotation_result(winner):
    global arena_plan
    if winner:
        if winner in ['Model-A 🟣', 'Model-B 🟤']:
            return f'Winner 🎖 is {winner}! \n ({arena_plan.progress()})'
        else:
            if 'Bad' in winner:
                performance = 'bad'
            ...
            return f'Their performance is {performance}! \n ({arena_plan.progress()})'
    else:
...
def arena_two_model(port):
    with gr.Blocks(title=emoji.emojize('Arena :crossed_swords:')) as demo:
        ...
        with gr.Row():
            radio = gr.Radio(['Model-A 🟣', 'Model-B 🟤', 'Draw (Bad) 🔴', 'Draw (Good) 🟢'], label='Competitor')
            result = gr.Label(label='Your decision 🎖')
        ...
        next_btn.click(fn=next_match, 
                       inputs=None, 
                       outputs=[instruction, response_a, response_b, radio, result], 
                       api_name='next')
        ...
        match_plan_btn.click(fn=load_match, 
                       ...
                       outputs=[plan_progress, instruction, response_a, response_b, radio, result], 
                       api_name='match_plan')
